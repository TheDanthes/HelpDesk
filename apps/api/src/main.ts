...
import { getTenantScope, isAdminOnlyRoute } from "./lib/tenant";
...
server.register(async (app) => {
  ...
  app.addHook("preHandler", async function (request: any, reply: any) {
    ...
    // Cierre global de la administracion para usuarios de cliente.
    // No depende de config.roles_active, que viene apagado por defecto.
    if (isAdminOnlyRoute(request.url, request.method)) {
      const scope = await getTenantScope(request);

      if (!scope || scope.kind !== "staff") {
        return reply.status(403).send({
          message: "Solo el personal de OnDesk puede acceder a esto",
          success: false,
        });
      }
    }
