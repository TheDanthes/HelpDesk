...
export function authRoutes(fastify: FastifyInstance) {
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Un usuario de cliente nace con sus empresas: sin membresias no ve nada.
      const memberships = Array.isArray(clients)
        ? clients
            .filter((c) => c && typeof c.clientId === "string")
            .map((c) => ({ clientId: c.clientId, viewAll: Boolean(c.viewAll) }))
        : [];
  ...
  fastify.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // requirePermission es no-op mientras roles_active este apagado, asi que
      // el borrado de usuarios se cierra explicitamente al personal de OnDesk.
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const data = {
        ...
        notifcations,
        external_user: user!.external_user,
        // Empresas a las que pertenece; alimenta el selector de empresa activa.
        clients: user!.clients
          .filter((m) => m.client.active)
          .map((m) => ({
            id: m.client.id,
            name: m.client.name,
            viewAll: m.viewAll,
  ...
  fastify.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // El email es una señal de pertenencia: tenantTicketWhere le muestra al
      // usuario los tickets que entraron desde su casilla. Si un usuario de
      // cliente pudiera cambiarselo, se apropiaria de los tickets de un
      // companero saltandose el limite de viewAll. Solo OnDesk cambia emails.
      const canChangeEmail = !session.external_user;

      let user = await prisma.user.update({
        where: { id: session.id },
        data: {
          name: name,
          email: canChangeEmail ? email : undefined,
          language: language,
        },
      });
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Cada uno cierra su propia sesion; forzar el logout de otro es de admin.
      if (session.id !== id && !session.isAdmin) {
        return reply.code(403).send({
          message: "Solo podes cerrar tu propia sesion",
          success: false,
        });
      }
  ...
  fastify.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      if (!session || session.id !== id) {
        return reply.code(403).send({
          message: "Solo podes marcar tu propio primer ingreso",
          success: false,
        });
      }
