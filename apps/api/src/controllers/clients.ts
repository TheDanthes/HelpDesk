...
import { requireStaffScope, requireTenantScope } from "../lib/tenant";
...
export function clientRoutes(fastify: FastifyInstance) {
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      // Un usuario de cliente solo ve las empresas donde tiene membresia.
      const clients = await prisma.client.findMany({
        where:
          scope.kind === "staff"
            ? {}
            : { id: { in: scope.memberships.map((m) => m.clientId) } },
        orderBy: { name: "asc" },
      });
